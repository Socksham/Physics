...
const AdvPhysics = ({ history }) => {
    ...
    useEffect(() => {
        ...
        async function func() {
            ...
            var arr = []
            await db.collection("class").doc("Physical Science").collection("topics").get().then((docs) => {
                docs.forEach((doc) => {
                    arr.push(doc.data())
    ...
    const getTopicDataBtn = async (topicName) => {
        ...
        const ref = db.collection("class").doc("Physical Science").collection("topics").doc(topicName).collection("days")
    ...
    return (
        <div className="bg-glass min-h-screen h-full pb-16">
            ...
            <div className="mt-10 mb-10">
                <p className="text-3xl md:text-5xl text-center">Physical Science</p>
            </div>
